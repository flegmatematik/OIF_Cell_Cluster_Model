...
directory = "output/31.3_one_cell/sim"+str(sim_id)
...
cell_type = oif.OifCellType(nodes_file="input/sphere1524nodes.dat",
...
# for i, cell in enumerate(cluster.cells):
    ...
    # cell.inner_particles.seed_sphere(system, r_part, n_part, 11 + i, particle_mass=1.0)
    # cell.inner_particles.set_interactions(system, dpd_gamma, hat_fmax, soft_a, soft_n, soft_cutoff=2*r_part)
...
print("high low points")
for id,part in enumerate(cluster.cells[0].mesh.points):
    pos = part.get_pos()
    if(high is None or pos[2] > high):
        high = pos[2]
        high_id = id
    if(low is None or pos[2] < low):
        low = pos[2]
        low_id = id
print(high_id)
print(high)
...
points_to_color = [[0.0 for i in range(cluster.cells[0].get_n_nodes())]]
points_to_color[0][high_id] = 1.0
points_to_color[0][low_id] = 1.0

cluster.cells[0].set_rotation()
...
# for id in range(ncells):
    # system.part.writevtk(
        # (vtk_directory + "/inner" + str(inner_particle_types[id]) + "_sim" + str(sim_id) + "_0.vtk"),
...
for cycle in range(1, maxCycle):
    ...
    angle = cluster.cells[0].get_rotation_angles()
    out_data = str(current_time)
    for a in angle:
        out_data += "," + str(a)
    
    out_file = open(directory + "/rot_new_" + str(sim_id) + ".txt", "a")
    out_file.write(out_data + "\n")
    out_file.close()
    ...
    out_data = str(current_time) + ","
    for id, cell in enumerate(cluster.cells):
        rotationX.append(velocities[id][0])
        rotationY.append(velocities[id][1])
        rotationZ.append(velocities[id][2])
    for r in rotationX:
