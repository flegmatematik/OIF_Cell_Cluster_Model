...
directory = "output/BiCluster_lj_shear_flow_innerpart/sim"+str(sim_id)
...
cluster.set_rotation([math.pi/4.0,0,0])
...
for i, cell in enumerate(cluster.cells):
    cell.seed_inner_particles(system, r_part, phi, inner_particle_types[i], particle_mass=particle_mass, dpd_gamma=dpd_gamma, hat_fmax=hat_fmax, soft_a=soft_a, soft_n=soft_n)
    print("writing vtk")
    for j in range(len(cell.inner_particles)):
        # REMOVE check written vtk
        if j % 200 == 0:
            print("parts_done: " + str(j))
        system.part.writevtk((vtk_directory + "/inner" + str(inner_particle_types[i]) + "_sim" + str(sim_id) + "_0" + ".vtk"), types=[(inner_particle_types[i])])
    print("done writing vtk")
    
    
ncells = len(cluster.cells)
print("Inner particles created")
# creating inner particles
for i in range(ncells):
    for j in range(inner_particle_counts[i]):
        system.part.writevtk((vtk_directory + "/inner" + str(inner_particle_types[i]) + "_sim" + str(sim_id) + "_0" + ".vtk"), types=[(inner_particle_types[i])])
print("Inner particles vtk created")
...
for cycle in range(1, maxCycle):
    ...
    # vtk 
    cluster.get_vtk_cluster(vtk_directory, cycle)
