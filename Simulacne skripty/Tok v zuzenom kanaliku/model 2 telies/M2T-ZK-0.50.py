...
cluster.load_cluster(system=system, directory='cluster_collection/Bicluster_lj/save_60',origin=[10.0,boxY/2.0+1.5,boxZ/2.0], load_interactions=False)
...
for i, cell in enumerate(cluster.cells):
    print("cell volume " + str(cell.volume()))
    n_part = int(phi*cell.volume()/part_volume)
    cell.inner_particles.seed_sphere(system, r_part, n_part, 11 + i, particle_mass=particle_mass, seed=1)
    cell.inner_particles.set_interactions(dpd_gamma, hat_fmax, soft_a, soft_n, soft_cutoff=2*r_part)

# rotate inner_particles of second cell
# cluster.cells[1].inner_particles.set_rotation([0.0,0.0,math.pi])

# invert inner particles of second cell
...
out_data = "time,x_vel_cell_1,y_vel_cell_1,z_vel_cell_1,x_pos_cell_1,y_pos_cell_1,z_pos_cell_1,x_vel_cell_2,y_vel_cell_2,z_vel_cell_2,x_pos_cell_2,y_pos_cell_2,z_pos_cell_2,cluster_x,cluster_y,cluster_z,pos_min_x,pos_max_x,pos_min_y,pos_max_y,pos_min_z,pos_max_z,pos_min_x,pos_max_x,pos_min_y,pos_max_y,pos_min_z,pos_max_z"
out_file.write(out_data + "\n")
out_file.close()
...
for cycle in range(1, maxCycle):
    ...
    #elongation
    pos_bounds_0 = cluster.cells[0].pos_bounds()
    pos_bounds_1 = cluster.cells[1].pos_bounds()     
            
    out_data += str(pos_bounds_0[0]) + "," + str(pos_bounds_0[1]) + "," + str(pos_bounds_0[2]) + "," + str(pos_bounds_0[3]) + "," + str(pos_bounds_0[4]) + "," + str(pos_bounds_0[5]) + ","
    
    out_data += str(pos_bounds_1[0]) + "," + str(pos_bounds_1[1]) + "," + str(pos_bounds_1[2]) + "," + str(pos_bounds_1[3]) + "," + str(pos_bounds_1[4]) + "," + str(pos_bounds_1[5])
